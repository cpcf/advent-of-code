...
def apply_instructions_cratemover_9000(crate_layout, converted_instructions):
  for instruction in converted_instructions:
    amount_to_move, move_from, move_to = instruction
    for i in range(amount_to_move):
      crate_layout[move_to - 1].insert(0, crate_layout[move_from - 1].pop(0))
  return crate_layout

def apply_instructions_cratemover_9001(crate_layout, converted_instructions):
  for instruction in converted_instructions:
    amount_to_move, move_from, move_to = instruction
    crates_on_the_move = crate_layout[move_from - 1][0:amount_to_move]
    del crate_layout[move_from - 1][0:amount_to_move]
    while crates_on_the_move:
      crate_layout[move_to - 1].insert(0, crates_on_the_move.pop())
  return crate_layout
...
crate_layout, converted_instructions =  prepare_input(input)

print('Part1: ' + get_top_layer(apply_instructions_cratemover_9000(copy.deepcopy(crate_layout), converted_instructions)))
print('Part2: ' + get_top_layer(apply_instructions_cratemover_9001(copy.deepcopy(crate_layout), converted_instructions)))